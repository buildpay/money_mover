...
describe MoneyMover::Dwolla::CustomerFundingSourceResource do
  ...
  let(:customer_funding_source_model) { MoneyMover::Dwolla::CustomerFundingSource.new(attrs) }
  ...
  describe '#create' do
    context 'valid' do
      ...
      it 'creates new customer funding source in dwolla' do
        expect(subject.create(customer_funding_source_model, customer_token)).to eq(true)
    ...
    context 'invalid' do
      ...
      it 'returns false and sets errors' do
        expect(subject.create(customer_funding_source_model, customer_token)).to eq(false)
